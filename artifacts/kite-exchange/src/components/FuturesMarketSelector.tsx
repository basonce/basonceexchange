...
function TradFiLogo({ asset, size }: { asset: TradFiAsset; size: number }) {
  ...
  if (asset.logoUrl?.includes('flagcdn.com')) {
    const bg = asset.bgColor ?? '#1a1a2a';
    return (
      <div className="flex-shrink-0 rounded-full overflow-hidden flex items-center justify-center" style={{ width: size, height: size, background: bg, border: '2px solid rgba(255,255,255,0.2)', boxShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>
