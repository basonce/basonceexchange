...
export default function TradePage({ onBack }: { onBack?: () => void }) {
  ...
  const safeHigh = (price: number, stored: number) => {
    ...
    if (stored > 0 && stored >= price && stored <= price * 1.5) return stored;
        ...
        <div className="grid grid-cols-[58%_42%] gap-0">
          <div className="bg-[#181A20] flex flex-col border-[#2B3139]">
            ...
            <div className="px-3 py-2 overflow-y-auto">
              ...
              {/* USDT Freeze Banner */}
              {(() => {
                if (!userRestrictions?.usdt_frozen) return null;
                const metalInfo = getMetalCross(selectedSymbol);
                if (metalInfo) return null; // BTC pairs are allowed
                return (
              ...
              {(() => {
                ...
                const isUsdtFrozen = !!userRestrictions?.usdt_frozen && !metalInfo;
                const isBlocked = isPairBlocked || isUsdtFrozen;
                return (
                  <button
                    onClick={handleTrade}
