...
// auth.lock: no-op — bypass the Web Locks API entirely.
// The default lock causes signInWithPassword() to block behind getSession()
// for up to 5-15 s whenever the access token needs refreshing.
// Our centralized _user cache (via onAuthStateChange) makes the lock unnecessary.
