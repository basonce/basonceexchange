...
// pure math. Mirrors the kite-exchange engine so prices stay perfectly in sync.
...
// Deterministic sparkline series ending "now" — for mini charts.
export function bncSparkline(points = 48, spanMs = 6 * 3600 * 1000, nowMs = Date.now()): number[] {
  const out: number[] = [];
