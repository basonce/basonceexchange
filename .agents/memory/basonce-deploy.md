...
- **`npx wrangler@4 --version` foreground often exceeds the 120s tool limit** (download stalls with no output). Workaround: launch it with `nohup ... &`, then poll `find /home/runner/.npm/_npx -name wrangler -type f` until the cached binary appears, and invoke that binary path directly. The `_npx` cache hash changes between sessions — don't hardcode it, glob for it.
