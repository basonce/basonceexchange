...
## basonce-wallet artifact is NOT deployed anywhere
- `basonce.com/wallet` returns 200 but it is just the kite-exchange SPA fallback route — the separate `artifacts/basonce-wallet` app exists only in the Replit dev preview. Netlify site + Cloudflare Pages both serve kite-exchange only.
- **Why:** avoids falsely claiming a wallet fix is "live"; wallet changes are only visible in the workspace preview until a hosting target is chosen for it.
