...
- **The build "transforming…" hang is NOT contention/detachment — it is Tailwind v4 auto content-scan.** See `tailwind-v4-build-hang.md`: fix is `@import "tailwindcss" source(none)` + scoped `@source` in `src/index.css` (already applied). Always run the build FOREGROUND under `timeout 110 …` (background `&` makes the tool wait for the descendant and falsely time out at 120s). With the fix the foreground build completes in ~51s. Then `cp cf-worker/_worker.js dist/public/_worker.js` and stage to `/tmp` for deploy.
